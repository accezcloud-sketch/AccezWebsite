        ...
        <div className="container mx-auto px-4">
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch">
              {/* Left side - one cohesive contact panel */}
              <FadeUp className="h-full">
                <div
                  className="rounded-2xl p-6 sm:p-8 h-full flex flex-col"
                  style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', boxShadow: '0 4px 24px rgba(0,0,0,0.3)' }}
                >
                  <h3 className="text-white font-semibold mb-1" style={{ fontSize: 20, fontFamily: 'var(--font-heading), var(--font-inter), system-ui, sans-serif' }}>
                    {tc.title}
                  </h3>
                  <p className="mb-6" style={{ fontSize: 14, color: 'var(--text-muted)' }}>{tc.subtitle}</p>
                  ...
                  <div className="flex flex-col gap-5 flex-1">
                    ...
                    <div className="flex items-start gap-3.5">
                      <div className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: 'var(--accent-dim)', border: '1px solid var(--accent-line)' }}>
                        <svg className="w-5 h-5" fill="none" stroke="var(--accent-hi)" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a2 2 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                      </div>
                      <div>
                        <h4 className="font-semibold text-white mb-0.5" style={{ fontSize: 14.5 }}>{tc.office}</h4>
                        <p style={{ fontSize: 14, color: 'var(--text-muted)' }}>{tc.officeDetails}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-3.5">
                      <div className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: 'var(--accent-dim)', border: '1px solid var(--accent-line)' }}>
                        <svg className="w-5 h-5" fill="none" stroke="var(--accent-hi)" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                      </div>
                      <div>
                        <h4 className="font-semibold text-white mb-0.5" style={{ fontSize: 14.5 }}>{tc.businessHours}</h4>
                        <p style={{ fontSize: 14, color: 'var(--text-muted)' }}>{tc.businessHoursDetails}</p>
                      </div>
                    </div>
                  </div>

                  {/* WhatsApp — themed to the dark UI, still clearly WhatsApp */}
                  <a
                    href="https://wa.me/18444603371"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="mt-6 inline-flex items-center justify-center gap-2.5 w-full px-5 py-3.5 rounded-xl font-semibold transition-all duration-200 hover:-translate-y-0.5"
                    style={{ background: 'rgba(37,211,102,0.12)', border: '1px solid rgba(37,211,102,0.35)', color: '#fff' }}
                  >
